...
function attachHandlers(root) {
    root.querySelectorAll('.rh-tab').forEach((tab) => {
        tab.addEventListener('click', () => {
            ...
            root.querySelectorAll('.rh-tab').forEach((t) => t.classList.toggle('active', t === tab));
            root.querySelectorAll('section[data-section]').forEach((sec) => {
                sec.hidden = sec.dataset.section !== which;
            });
        });
    });
    const fileInput = root.querySelector('#rh-file-input');
    if (fileInput) {
        fileInput.addEventListener('change', (e) => importFiles(e.target.files, root));
    }
    ...
    const modeSel = root.querySelector('#rh-mode');
    const dirSel = root.querySelector('#rh-direction');
    const bandSel = root.querySelector('#rh-band');
    if (modeSel)
        modeSel.addEventListener('change', () => updateStateAndRender(root));
    if (dirSel)
        dirSel.addEventListener('change', () => updateStateAndRender(root));
    if (bandSel)
        bandSel.addEventListener('change', () => updateStateAndRender(root));
...
async function importFiles(files, root) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        const parsed = parseRunHealthCsv(text, f.name);
        if (parsed.kind === 'ok') {
...
function updateStateAndRender(root) {
    ...
    const newState = {
        ...ctx.state,
        selectedDirection: dirSel?.value ?? 'BOTH',
        selectedPowerBand: bandSel?.value ?? 'ALL_ACTIVE',
...
function populateSelect(sel, runs) {
    if (!sel)
        return;
...
function renderWholeTable(root) {
    ...
    if (sel.value === 'DIRECT') {
        ...
        reference = r === undefined ? null : r;
        const c = ctx.runs.get(cmpSel.value);
        comparison = c === undefined ? null : c;
    }
    else {
        const baseId = baseSel.value || ctx.state.baselineRunId;
        const baseline = baseId && ctx.runs.has(baseId)
            ? ctx.runs.get(baseId)
            : null;
        ...
        comparison = candidates.length > 0 ? candidates[0] : null;
    }
    table.innerHTML = '';
    ...
    // size manageable.
    const direction = (dirSel?.value ?? 'BOTH');
    const band = (bandSel?.value ?? 'ALL_ACTIVE');
    ...
    for (const d of devices) {
        ...
        if (!refHas && !cmpHas)
            status = 'Missing';
        else if (!refHas || !cmpHas)
            status = 'Missing in Comparison';
        else if (refCount < 20 || cmpCount < 20)
...
function countFilteredSamples(run, device, direction, band) {
    let count = 0;
    const low = 0.15, med = 0.35, high = 0.65, max = 1.0;
    for (const s of run.samples) {
        if (s.deviceName !== device)
            continue;
        if (s.commandedPower === null)
            continue;
        if (direction === 'POSITIVE' && s.commandedPower <= 0)
            continue;
        if (direction === 'NEGATIVE' && s.commandedPower >= 0)
            continue;
        const abs = Math.abs(s.commandedPower);
        if (band === 'LOW' && (abs < low || abs >= med))
            continue;
        if (band === 'MEDIUM' && (abs < med || abs >= high))
            continue;
        if (band === 'HIGH' && (abs < high || abs > max))
            continue;
        if (band === 'ALL_ACTIVE' && abs < low)
            continue;
        count++;
    }
    return count;
