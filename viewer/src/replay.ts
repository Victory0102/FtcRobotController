...
import { ChannelSample } from './types.js';
 ...
 */
export function latestIndexAtOrBefore(
  ...
  while (lo <= hi) {
    const mid = (lo + hi) >>> 1;
    if (samples[mid].timestampMs <= t) { best = mid; lo = mid + 1; }
    else hi = mid - 1;
  }
  return best;
}
 ...
 */
export function lookupPose(
  samples: readonly ChannelSample[],
  t: number,
  maxGapMs: number = DEFAULT_MAX_GAP_MS,
): { x: number; y: number; heading: number | null; atMs: number } | null {
  const idx = latestIndexAtOrBefore(samples, t);
  if (idx === -1) return null;
  const cur = samples[idx];
  if (cur.kind !== 'pose') return null;
  if (cur.valueX === null || cur.valueY === null) return null;
  if (!Number.isFinite(cur.valueX) || !Number.isFinite(cur.valueY)) return null;
...
function interpolateHeading(a: number | null, b: number | null, frac: number): number | null {
  if (a === null || b === null) return a ?? b;
  // We do not interpolate heading mod 2pi here; consumers can re-normalise.
  return a + (b - a) * frac;
}

function numericValueOf(s: ChannelSample): number | null {
  if (s.kind === 'number') {
    if (s.valueNumber === null) return null;
    return Number.isFinite(s.valueNumber) ? s.valueNumber : null;
  }
  if (s.kind === 'pose') {
    if (s.valueX === null) return null;
    return Number.isFinite(s.valueX) ? s.valueX : null;
  }
  return null;
}

/* ====================== Replay clock + consumer pattern =============== */

export type ReplayConsumer = (timeMs: number) => void;

export interface ReplaySnapshot {
  /** Replay time in milliseconds since run start. */
  timeMs: number;
  /** Wall-clock timestamp at which the snapshot was taken. */
  wallMs: number;
  isPlaying: boolean;
  speed: number;
  /** 0 to 1, fraction complete (clamped). */
  progress: number;
}
...
export class ReplayClock {
  ...
  private startMs = 0;
  private endMs = 0;
  private consumers = new Set<ReplayConsumer>();

  /** Load a run's duration bounds.  Calling this resets timeMs to 0. */
  loadRun(startMs: number, endMs: number): void {
  ...
  registerConsumer(c: ReplayConsumer): () => void {
    this.consumers.add(c);
    try { c(this.timeMs); } catch (e) { /* swallow - consumer fault */ }
  ...
  play(): void {
    ...
    if (this.timeMs >= this.endMs - this.startMs) this.timeMs = 0;
    this.isPlaying = true;
    this.lastWallMs = 0;
    this.startRaf();
  }
  ...
  isPlayingNow(): boolean { return this.isPlaying; }
  getTime(): number { return this.timeMs; }
  getDuration(): number { return Math.max(0, this.endMs - this.startMs); }
  ...
  private notify(): void {
    for (const c of this.consumers) {
      try { c(this.timeMs); } catch (e) { /* consumer fault is not propagated */ }
    }
  }
}
