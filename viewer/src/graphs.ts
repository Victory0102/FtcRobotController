...
import { Sample, Series, SeriesDomain, SeriesPoint } from './types.js';
 ...
 * has zero points.
 */
export function seriesDomain(seriesList: readonly Series[]): SeriesDomain | null {
...
export function timeToX(
  t: number, domain: SeriesDomain, widthPx: number,
): number {
  if (domain.tMax === domain.tMin) return 0;
  return ((t - domain.tMin) / (domain.tMax - domain.tMin)) * widthPx;
}

/** Map a y-value to a canvas y-pixel given a value-domain. */
export function valueToY(
  v: number, domain: SeriesDomain, heightPx: number,
): number {
  if (domain.yMax === domain.yMin) return heightPx / 2;
...
export function yToValue(
  yPx: number, domain: SeriesDomain, heightPx: number,
): number {
  if (domain.yMax === domain.yMin) return domain.yMin;
 ...
 */
export function hoverLookup(
  seriesList: readonly Series[],
  t: number,
): { series: Series; point: SeriesPoint }[] {
  const out: { series: Series; point: SeriesPoint }[] = [];
  for (const series of seriesList) {
    if (!series.visible || series.points.length === 0) continue;
    // Binary search for the largest index where points[i].t <= t.
    let lo = 0, hi = series.points.length - 1, best = -1;
    while (lo <= hi) {
      const mid = (lo + hi) >>> 1;
      if (series.points[mid].t <= t) { best = mid; lo = mid + 1; }
      else hi = mid - 1;
    }
    if (best === -1) continue;
...
export function motorMetricSeries(
  motorName: string,
  samples: readonly Sample[],
  index: number,
): Series[] {
  return [
    ...
    makeSeries(`${motorName}.current`, `${motorName} – current`,
        buildSampleSeries(samples, (s) => s.currentAmps), index),
  ];
}
