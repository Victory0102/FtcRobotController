...
import { Run, ChronoPoint } from './types.js';
import { perMotorTrend, TrendRow } from './comparison.js';
import { DEFAULT_THRESHOLDS, FilterSelection, StatusThresholds } from './types.js';

/** Series keys shipped in the chronological trend view.  Each maps to a
 *  builder that emits ChronoPoint[] from a list of runs. */
...
export interface CustomChannelTrendSpec {
  channelName: string;
  /** Optional pre-computed summary list, one per run. */
  summariesByRun?: Map<string, { median: number | null; sampleCount: number }>;
}

/* ============== Motor-metric trend builders ========================== */

/** Build median velocity trend rows.  Requires the chosen motor name. */
export function medianVelocityTrend(
  runs: readonly Run[],
  motorName: string,
  filter: FilterSelection,
  thresholds: StatusThresholds = DEFAULT_THRESHOLDS,
): ChronoPoint[] {
  return buildChronoFromTrendRows(
    perMotorTrend(runs, motorName, filter, thresholds),
    runs,
    (r) => r.medianVelocity,
  );
}

export function velocityEfficiencyTrend(
  runs: readonly Run[],
  motorName: string,
  filter: FilterSelection,
  thresholds: StatusThresholds = DEFAULT_THRESHOLDS,
  ...
  return buildChronoFromTrendRows(
    perMotorTrend(runs, motorName, filter, thresholds),
    runs,
    (r) => r.velocityEfficiency,
  );
}

export function currentCostTrend(
  runs: readonly Run[],
  motorName: string,
  filter: FilterSelection,
  thresholds: StatusThresholds = DEFAULT_THRESHOLDS,
...
export function p95CurrentTrend(
  runs: readonly Run[],
  motorName: string,
  filter: FilterSelection,
  thresholds: StatusThresholds = DEFAULT_THRESHOLDS,
...
export function possibleStallPctTrend(
  runs: readonly Run[],
  motorName: string,
  filter: FilterSelection,
  thresholds: StatusThresholds = DEFAULT_THRESHOLDS,
...
export function batteryMinTrend(runs: readonly Run[]): ChronoPoint[] {
  return runs.map((run) => {
    let min: number | null = null;
    for (const s of run.samples) {
      if (s.batteryVoltage === null) continue;
      if (!Number.isFinite(s.batteryVoltage) || s.batteryVoltage <= 0) continue;
      if (min === null || s.batteryVoltage < min) min = s.batteryVoltage;
    }
...
export function batteryMedianTrend(runs: readonly Run[]): ChronoPoint[] {
  return runs.map((run) => {
 ...
 */
export function loopTimeTrend(runs: readonly Run[]): {
  median: ChronoPoint[];
  p95: ChronoPoint[];
} {
  const medianPts: ChronoPoint[] = [];
  const p95Pts: ChronoPoint[] = [];
 ...
 * gap).
 */
export function customChannelTrend(
  runs: readonly Run[],
  channelName: string,
): ChronoPoint[] {
  return runs.map((run) => {
    const chs = run.channels?.byChannel[channelName.toLowerCase()];
    if (!chs || chs.length === 0) return makeBasePoint(run, null, true);
    const values: number[] = [];
    for (const ch of chs) {
      if (ch.kind !== 'number') continue;
      if (ch.valueNumber === null) continue;
      if (Number.isFinite(ch.valueNumber)) values.push(ch.valueNumber);
    }
    if (values.length < 1) return makeBasePoint(run, null, true);
    return makeBasePoint(run, medianOf(values), false);
  }).sort(byRunStartedAt);
}

/* ============== Baseline / previous deltas =========================== */

export interface DeltaOptions {
  /** Optional baseline runId; deltas are left null for every point when absent. */
...
function buildChronoFromTrendRows(
  rows: TrendRow[],
  runs: readonly Run[],
...
function byRunStartedAt(a: ChronoPoint, b: ChronoPoint): number {
