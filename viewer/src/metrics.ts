...
export function filterSamples(
  ...
  restrictMode: string | null = null,
): { matching: Sample[]; motorModeMismatch: boolean } {
  const matching: Sample[] = [];
  // Track distinct non-null motor modes we encounter.
  const modes = new Set<string>();
  for (const s of samples) {
    if (s.deviceName !== deviceName) continue;
    ...
    if (s.motorMode !== null) modes.add(s.motorMode);
    if (!directionMatches(s.commandedPower, filter.direction)) continue;
    const abs = s.commandedPower === null ? 0 : Math.abs(s.commandedPower);
    if (!powerBandMatches(abs, filter.powerBand)) continue;
    matching.push(s);
  }
...
function directionMatches(commanded: number | null, dir: Direction): boolean {
  if (commanded === null) return false;
  if (dir === 'BOTH') return true;
  if (dir === 'POSITIVE') return commanded > 0.0;
  if (dir === 'NEGATIVE') return commanded < 0.0;
  return false;
}

function powerBandMatches(absPower: number, band: PowerBand): boolean {
  if (band === 'ALL_ACTIVE') {
 ...
 * exist.
 */
export function medianAbsoluteVelocity(
  filtered: readonly Sample[],
  minSamples: number = DEFAULT_THRESHOLDS.minSamples,
): { value: number | null; n: number; activeMs: number } {
  const values: number[] = [];
  for (const s of filtered) {
    if (s.encoderVelocity === null) continue;
 ...
 */
export function effectiveCommand(s: Sample): number | null {
  if (s.commandedPower === null) return null;
  if (s.batteryVoltage === null) return null;
  const absP = Math.abs(s.commandedPower);
  if (absP <= 0) return null;
 ...
 * effective command.
 */
export function velocityPerEffectiveCommand(
  filtered: readonly Sample[],
  minSamples: number = DEFAULT_THRESHOLDS.minSamples,
 ...
 */
export function currentPerMovement(
  filtered: readonly Sample[],
  minVelocityTps: number = DEFAULT_THRESHOLDS.currentCostMinVelocityTps,
  minSamples: number = DEFAULT_THRESHOLDS.minSamples,
): { value: number | null; n: number } {
  const values: number[] = [];
  for (const s of filtered) {
    if (s.currentAmps === null) continue;
    if (s.encoderVelocity === null) continue;
    const v = Math.abs(s.encoderVelocity);
    if (v < minVelocityTps) continue;
    // amps per 1,000 tps: I * 1000 / |v|
    values.push(s.currentAmps * 1000 / v);
  }
  if (values.length < minSamples) return { value: null, n: values.length };
  return { value: median(values), n: values.length };
}
 ...
 * false positives.
 */
export function possibleStallPercentage(
  filtered: readonly Sample[],
  ...
  for (const s of filtered) {
    if (s.commandedPower === null || s.encoderVelocity === null) continue;
    ...
    if (Math.abs(s.commandedPower) >= thresholds.stallPowerThreshold) active++;
  }
 ...
 */
export function activeDuration(samples: readonly Sample[]): number {
...
export function sampleCount(samples: readonly Sample[]): number {
...
export function classifyChange(
  ...
  available: boolean,
  thresholds: StatusThresholds = DEFAULT_THRESHOLDS,
): 'Stable' | 'Changed' | 'Large Change' | 'Insufficient Data' {
  if (!available || pct === null) return 'Insufficient Data';
  const ap = Math.abs(pct);
  if (ap >= thresholds.largeChangePct) return 'Large Change';
  if (ap >= thresholds.changedPct) return 'Changed';
  return 'Stable';
}
...
export function describePowerBand(band: PowerBand): string {
