...
import {
  formatPower, formatTps, formatAmps,
...
interface AppContext {
  ...
  /* Indexed by runId; each entry is a freshly-prepared ReplayState for that
...
interface ReplaySlice {
  run: Run;
  /* Per-channel pre-prepared arrays. */
  byChannelSamples: Map<string, ChannelSample[]>;
  /* Cached bounds for the replay clock. */
  endMs: number;
}
...
const TABS: { id: string; label: string; group: string }[] = [
  ...
  const tabs = groups.map((group) => `
    <div class="rh-nav-group">
      ...
      ${TABS.filter((tab) => tab.group === group).map((tab) =>
        `<button data-tab="${tab.id}" class="rh-tab${tab.id === 'live' ? ' active' : ''}"><span>${escapeHtml(tab.label)}</span><span class="rh-tab__mark" aria-hidden="true"></span></button>`
      ).join('')}
    </div>
  `).join('');
  root.innerHTML = `
...
function attachHandlers(root: HTMLElement) {
  ...
  root.querySelector<HTMLInputElement>('#rh-file-input')
    ?.addEventListener('change', (e) => importFiles((e.target as HTMLInputElement).files, root));
  root.querySelector<HTMLButtonElement>('#rh-reset')
  ...
  const dnd = root.querySelector<HTMLElement>('#rh-import-dnd');
  const dndBrowse = root.querySelector<HTMLButtonElement>('#rh-import-browse');
  const dndFileInput = root.querySelector<HTMLInputElement>('#rh-file-input');
  ...
  root.querySelector<HTMLButtonElement>('#rh-import-open')
    ?.addEventListener('click', () => {
      const ctx = getCtx(root);
  ...
  root.querySelector<HTMLButtonElement>('#rh-replay-play')
    ?.addEventListener('click', () => globalReplayClock.play());
  root.querySelector<HTMLButtonElement>('#rh-replay-pause')
    ?.addEventListener('click', () => globalReplayClock.pause());
  root.querySelector<HTMLButtonElement>('#rh-replay-restart')
    ?.addEventListener('click', () => globalReplayClock.restart());
  root.querySelector<HTMLSelectElement>('#rh-replay-speed')
    ?.addEventListener('change', (e) => {
      globalReplayClock.setSpeed(Number((e.target as HTMLSelectElement).value));
    });
  root.querySelector<HTMLInputElement>('#rh-replay-scrubber')
...
function sectionSwitch(root: HTMLElement, which: string) {
  root.querySelectorAll<HTMLElement>('.rh-tab').forEach((t) =>
    t.classList.toggle('active', t.dataset.tab === which));
  root.querySelectorAll<HTMLElement>('section[data-section]').forEach((s) => {
...
function renderSavedSection(root: HTMLElement) {
  const ctx = getCtx(root);
  ...
  table.querySelectorAll<HTMLButtonElement>('button[data-act]').forEach((btn) => {
 ...
 */
function renderCompareCards(root: HTMLElement) {
  const container = root.querySelector<HTMLElement>('#rh-compare-cards');
  if (!container) return;
  const ctx = getCtx(root);
  ...
  let ref: Run | null = null, cmp: Run | null = null;
  if (sel.value === 'DIRECT') {
  ...
  const filter: FilterSelection = {
    direction: (dirSel?.value as any) ?? 'BOTH',
    powerBand: (bandSel?.value as any) ?? 'ALL_ACTIVE',
  };
  if (!ref || !cmp) {
...
function renderCompareSection(root: HTMLElement) {
  const ctx = getCtx(root);
  const sel = root.querySelector<HTMLSelectElement>('#rh-mode')!;
  ...
  if (sel.value === 'DIRECT') {
    refRow.hidden = false; cmpRow.hidden = false; baseRow.hidden = true;
  } else {
    refRow.hidden = true; cmpRow.hidden = true; baseRow.hidden = false;
  }
 ...
 */
function renderCompareSummaryStrip(root: HTMLElement) {
  const ctx = getCtx(root);
  const strip = root.querySelector<HTMLElement>('#rh-compare-summary');
  if (!strip) return;
  const sel = root.querySelector<HTMLSelectElement>('#rh-mode')!;
  const baseSel = root.querySelector<HTMLSelectElement>('#rh-baseline')!;
  const refSel = root.querySelector<HTMLSelectElement>('#rh-reference')!;
  ...
  for (const chip of summary.chips) {
    ...
    if (chip.statusClass) value.className = `rh-status ${chip.statusClass}`;
    else value.className = 'rh-metric__value';
...
function renderWholeTable(root: HTMLElement) {
  const ctx = getCtx(root);
...
const COMPARISON_HEADERS: readonly string[] = [
...
function signalsShort(r: { hasCurrentRef: boolean; hasCurrentCmp: boolean;
                             hasVoltageRef: boolean; hasVoltageCmp: boolean; }): string {
...
function attachReplayRun(root: HTMLElement, runId: string) {
  if (!runId) return;
  const ctx = getCtx(root);
...
function renderReplaySection(root: HTMLElement) {
  const ctx = getCtx(root);
  ...
  if (time) time.textContent = `${cur|0} / ${totals} ms`;
  ...
  const update = (t: number) => {
    if (time) time.textContent = `${t|0} / ${totals} ms`;
    if (scrub && globalReplayClock.getDuration() > 0) {
...
function renderReplayCanvas(root: HTMLElement, t: number) {
  const ctx = getCtx(root);
  const canvas = root.querySelector<HTMLCanvasElement>('#rh-replay-canvas');
  if (!canvas) return;
  ...
  const series: { id: string; label: string; color: string; visible: boolean;
                    points: { t: number; v: number; gap: boolean }[] }[] = [];
  for (const device of slice.run.deviceNames) {
    const samps = slice.run.samples.filter((x) =>
      x.deviceName === device && x.timestampMs >= startMs && x.timestampMs <= t);
    const m = motorMetricSeries(device, samps, 0);
    for (const s of m) series.push(s);
  }
...
function renderReplayValues(root: HTMLElement, t: number) {
  const ctx = getCtx(root);
  const table = root.querySelector<HTMLTableElement>('#rh-replay-values');
  if (!table) return;
  const sel = root.querySelector<HTMLSelectElement>('#rh-replay-run');
  const runId = sel?.value ?? '';
  const slice = ctx.replayByRun.get(runId);
  ...
  const rows: { name: string; kind: string; value: string }[] = [];
...
function renderChannelsSection(root: HTMLElement) {
  const ctx = getCtx(root);
  ...
  for (const channelName of run.channels.channelNames) {
    ...
    if (chs[0]?.kind === 'number') {
      const v = chs.map((x) => x.valueNumber).filter((x) => x !== null) as number[];
      ...
      let prevT = NaN;
      for (const x of chs) {
        if (x.valueBoolean === null) continue;
        if (prev !== null && x.valueBoolean !== prev) trans++;
        if (x.valueBoolean === true) timeTrue += Math.max(0, x.timestampMs - prevT);
        prev = x.valueBoolean;
        prevT = x.timestampMs;
      }
      ...
      const transList: { at: number; value: string }[] = [];
      let prevText: string | null = null;
      for (const x of chs) {
        if (x.valueText === null) continue;
 ...
 * textContent. */
function makeTable(rows: [string, string][]): HTMLTableElement {
...
function lastNumeric(chs: ChannelSample[]): number | null {
  for (let i = chs.length - 1; i >= 0; i--) {
    const s = chs[i];
    if (s && s.valueNumber !== null && s.valueNumber !== undefined) return s.valueNumber;
  }
  return null;
}
function lastBoolean(chs: ChannelSample[]): boolean | null {
  for (let i = chs.length - 1; i >= 0; i--) {
    const s = chs[i];
    if (s && s.valueBoolean !== null && s.valueBoolean !== undefined) return s.valueBoolean;
  }
  return null;
}
function lastText(chs: ChannelSample[]): string | null {
  for (let i = chs.length - 1; i >= 0; i--) {
    const s = chs[i];
    if (s && s.valueText !== null && s.valueText !== undefined) return s.valueText;
  }
  return null;
}
...
function buildTrendSeries(
  key: string, runs: Run[], motor: string, filter: FilterSelection,
): ChronoPoint[] {
  const runById = new Map<string, Run>(runs.map((r) => [r.runId, r]));
...
function batteryMinPoints(runs: Run[]): ChronoPoint[] {
  return runs.map((r) => {
    let mn: number | null = null;
    for (const s of r.samples) {
      if (s.batteryVoltage === null) continue;
...
function batteryMedianPoints(runs: Run[]): ChronoPoint[] {
  return runs.map((r) => {
    const v: number[] = [];
    for (const s of r.samples) {
      if (s.batteryVoltage === null) continue;
      if (Number.isFinite(s.batteryVoltage) && s.batteryVoltage > 0) v.push(s.batteryVoltage);
    }
    if (v.length < 5) return { runId: r.runId, runStartedAt: r.runStartedAt,
...
function loopTimeHelper(runs: Run[], p: number): ChronoPoint[] {
  return runs.map((r) => {
    const dt: number[] = [];
    ...
    if (dt.length < 5) return { runId: r.runId, runStartedAt: r.runStartedAt,
      opmodeName: r.opmodeName, buildIdentifier: r.buildIdentifier ?? '',
      value: null, baselineDelta: null, previousDelta: null, insufficient: true };
...
function loopTimeMedianPoints(runs: Run[]): ChronoPoint[] { return loopTimeHelper(runs, 0.5); }
function loopTimeP95Points(runs: Run[]): ChronoPoint[] { return loopTimeHelper(runs, 0.95); }
 ...
 */
function renderImportSummary(root: HTMLElement) {
  const ctx = getCtx(root);
  const rejected: Array<{ filename: string; ok: false; sizeKb: number; reason: string }> = [];
  const list = root.querySelector<HTMLUListElement>('#rh-import-list');
  if (list) {
  ...
  const recognized = root.querySelector<HTMLElement>('#rh-import-recognized');
  const runs = root.querySelector<HTMLElement>('#rh-import-runs');
  const warnings = root.querySelector<HTMLElement>('#rh-import-warnings');
  if (recognized) recognized.textContent = String(summary.recognizedFiles);
  if (runs) runs.textContent = String(summary.runCount);
  if (warnings) warnings.textContent = String(summary.warningCount);
  const openBtn = root.querySelector<HTMLButtonElement>('#rh-import-open');
  if (openBtn) {
...
function appendCell(tr: HTMLTableRowElement, text: string, klass?: string) {
  const td = tr.insertCell();
  td.textContent = text;
  if (klass) td.className = klass;
}
function runInfoLabel(run: Run, isBaseline: boolean): string {
  if (!run) return '';
...
function populateSelect(sel: HTMLSelectElement | null, runs: Map<string, Run>, devicesOnly = false) {
  if (!sel) return;
...
interface LiveUiState {
  ...
  /** Most-recent accepted snapshot, if any.  Used by the helper that builds
   *  the Live summary chips so a "no-snapshot" gap doesn't lose the prior
   *  opMode / session-id / battery reading for too long. */
  lastSnap: import('./types.js').LiveSnapshot | null;
  connectionState: 'connected' | 'connecting' | 'disconnected' | 'no_session';
  connectionStateAtMs: number;
}

const liveState: LiveUiState = {
...
function renderLiveSection(root: HTMLElement) {
  // Build (or replace) the Live section container.
  let el = root.querySelector('section[data-section="live"]') as HTMLElement | null;
  if (!el) {
...
function renderLiveDashboard(s: LiveUiState) {
  ...
  const liveStoreLike = s.store as unknown as LiveLikeStore;
  const snapLike = s.lastSnap as LiveLikeSnap | null;
  ...
  el.querySelectorAll<HTMLCanvasElement>('canvas[data-live-metric]').forEach((canvas) => {
