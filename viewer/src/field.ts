 ...
 */
export function fieldXToScreen(xIn: number, screenWidth: number): number {
 ...
 */
export function fieldYToScreen(yIn: number, screenHeight: number): number {
  return ((FIELD_HALF_IN - yIn) / FIELD_SIZE_IN) * screenHeight;
}

export interface ZoomPanState {
  /** Horizontal zoom; 1.0 = full field fits exactly. >1 zooms in. */
  zoomX: number;
  /** Vertical zoom. */
  zoomY: number;
  /** Pan offset in field-inches.  Positive offsets centre the field on
   *  a region offset upwards/rightwards. */
  panX: number;
  panY: number;
  /** When true, the renderer auto-fits so all content stays in view. */
  fitToField: boolean;
}
...
export function fieldToScreen(
  ...
  width: number,
  height: number,
  zp: ZoomPanState = DEFAULT_ZOOM_PAN,
): { sx: number; sy: number } {
  if (zp.fitToField) {
    return {
 ...
 */
export function fieldHeadingToScreen(headingRad: number): number {
...
export function isInsideField(xIn: number, yIn: number): boolean {
 ...
 */
export function clipPathToBoundary(
  points: readonly { x: number; y: number; tMs?: number }[],
): ({ x: number; y: number; tMs?: number | undefined } | null)[] {
  const out: ({ x: number; y: number; tMs?: number | undefined } | null)[] = [];
 ...
 */
export function computeBounds(points: readonly { x: number; y: number }[]): PathBounds | null {
...
export interface DrawFieldOptions {
  zp?: ZoomPanState;
  path?: { x: number; y: number; tMs?: number }[];
  /** When set, renders a marker at this pose and a heading line. */
  robot?: { x: number; y: number; headingRad?: number } | null;
}
...
export function drawField(
  ...
  for (let i = -FIELD_HALF_IN; i <= FIELD_HALF_IN; i += TILE_SIZE_IN) {
    if (i === -FIELD_HALF_IN || i === FIELD_HALF_IN) continue;
 ...
 */
export function poseSeriesToPath(
  samples: readonly ChannelSample[],
  gapThresholdMs: number = 500,
): { x: number; y: number; tMs: number; gap: boolean }[] {
  const out: { x: number; y: number; tMs: number; gap: boolean }[] = [];
  let prevT: number | null = null;
  for (const s of samples) {
    if (s.kind !== 'pose') continue;
    if (s.valueX === null || s.valueY === null) continue;
    if (!Number.isFinite(s.valueX) || !Number.isFinite(s.valueY)) continue;
