...
import { DEFAULT_THRESHOLDS, FilterSelection, Run, Sample } from './types.js';

export type WearSeverity = 'healthy' | 'watch' | 'warning' | 'insufficient';
export type WearGraph = 'power' | 'velocity' | 'current' | 'response' | 'overall';

export interface PowerResponseBin {
  power: number;
  sampleCount: number;
...
export interface MotorWearReport {
  motor: string;
  ...
  stallChangePoints: number | null;
  batteryDeltaV: number | null;
  modeMismatch: boolean;
  referenceBins: PowerResponseBin[];
  comparisonBins: PowerResponseBin[];
...
export interface WearHistoryPoint {
  runId: string;
  runStartedAt: string;
  healthScore: number | null;
  responseChangePct: number | null;
  currentChangePct: number | null;
  confidence: MotorWearReport['confidence'];
}

const BIN_WIDTH = 0.1;
const MIN_BIN_SAMPLES = 5;

function finiteMedian(values: Array<number | null>): number | null {
  return median(values.filter((v): v is number => v !== null && Number.isFinite(v)));
}

function normalizedResponse(sample: Sample): number | null {
  if (sample.commandedPower === null || sample.encoderVelocity === null) return null;
  const command = Math.abs(sample.commandedPower);
  if (command < 0.1) return null;
...
export function buildPowerResponse(
  run: Run,
  motor: string,
  filter: FilterSelection,
  minSamples = MIN_BIN_SAMPLES,
): PowerResponseBin[] {
  ...
  for (const sample of matching) {
    if (sample.commandedPower === null || sample.encoderVelocity === null) continue;
    const power = Math.abs(sample.commandedPower);
    if (power < 0.1 || power > 1.001) continue;
    const index = Math.min(9, Math.floor((power + 1e-9) / BIN_WIDTH) - 1);
    const bucket = buckets.get(index) ?? [];
    bucket.push(sample);
    buckets.set(index, bucket);
  }
  const out: PowerResponseBin[] = [];
  for (const [index, samples] of buckets) {
    ...
    if (velocity === null || response === null) continue;
    out.push({
...
function weightedChange(
  ref: PowerResponseBin[],
  cmp: PowerResponseBin[],
  pick: (bin: PowerResponseBin) => number | null,
): { value: number | null; bins: number; samples: number } {
  ...
  for (const a of ref) {
    ...
    const av = pick(a);
    const bv = pick(b);
    if (av === null || bv === null || !Number.isFinite(av) || !Number.isFinite(bv) || Math.abs(av) < 1e-9) continue;
...
function motorModes(run: Run, motor: string): Set<string> {
  return new Set(run.samples
    .filter((s) => s.deviceName === motor && s.motorMode)
    .map((s) => s.motorMode as string));
}

function medianBattery(run: Run, motor: string): number | null {
...
export function compareMotorWear(
  reference: Run,
  comparison: Run,
  motor: string,
  filter: FilterSelection,
  ...
  let confidence: MotorWearReport['confidence'] = 'low';
  ...
  const diagnoses: GraphDiagnosis[] = [];
  if (response.value === null) {
    diagnoses.push(diagnosis('power', 'insufficient', 'Command coverage does not overlap',
      'The runs do not contain enough samples in the same power bands.',
      'A speed difference would not be an apples-to-apples comparison.',
      'Repeat the same test route with similar commands, battery state, payload, and floor surface.'));
  } else {
    diagnoses.push(diagnosis('power', response.bins >= 3 ? 'healthy' : 'watch', 'Matched command coverage',
      `${response.bins} power bands overlap across the two runs.`,
      response.bins >= 3 ? 'The response comparison controls for commanded power.' : 'The result is sensitive to a narrow command range.',
      response.bins >= 3 ? 'Keep using the same repeatable test route.' : 'Exercise low, medium, and high power during the next test.'));
  }

  if (velocity.value === null) {
    diagnoses.push(diagnosis('velocity', 'insufficient', 'Velocity comparison unavailable',
      'There are not enough matched velocity samples.', 'No motor-speed conclusion is justified yet.',
      'Verify encoders report velocity, then repeat a consistent drive test.'));
  } else if (velocity.value <= -15) {
    diagnoses.push(diagnosis('velocity', 'warning', 'Motor speed dropped at matched commands',
      `Median velocity changed ${pct(velocity.value)} after matching power bands.`,
      'This can indicate added mechanical resistance, drivetrain loading, motor or gearbox wear, encoder issues, or different test conditions.',
      'Lift the robot safely, check wheel/shaft freedom, inspect gears and bearings, verify encoder wiring, then repeat under the same load.'));
  } else if (velocity.value <= -8) {
    diagnoses.push(diagnosis('velocity', 'watch', 'Motor speed is trending lower',
      `Median velocity changed ${pct(velocity.value)} at matched commands.`,
      'The change is worth tracking but is not enough by itself to identify a fault.',
      'Inspect for rubbing or binding and repeat the same test to confirm the trend.'));
  } else {
    diagnoses.push(diagnosis('velocity', 'healthy', 'Velocity response is stable',
      `Median velocity changed ${pct(velocity.value)} at matched commands.`,
      'No meaningful loss of speed is visible under the selected conditions.',
      'Continue collecting the same test periodically to establish a stronger history.'));
  }

  if (current.value === null) {
    diagnoses.push(diagnosis('current', 'insufficient', 'Current comparison unavailable',
      'One or both runs do not contain enough motor-current samples.',
      'Velocity-based diagnosis still works, but electrical and friction clues are weaker.',
      'Confirm the installed hub and motor port expose current sensing, then collect another run.'));
  } else if (current.value >= 20) {
    diagnoses.push(diagnosis('current', 'warning', 'Current demand increased',
      `Median current changed ${pct(current.value)} in matched power bands.`,
      'Higher current for the same command can accompany friction, binding, heavier load, wheel contact, or electrical problems.',
      'Check drivetrain freedom, wheel alignment, wiring and connectors, and compare again with the same payload.'));
  } else if (current.value >= 10) {
    diagnoses.push(diagnosis('current', 'watch', 'Current demand is trending higher',
      `Median current changed ${pct(current.value)} in matched power bands.`,
      'A developing load or friction change is possible, especially if velocity is also falling.',
      'Inspect the local mechanism and repeat the test before replacing parts.'));
  } else {
    diagnoses.push(diagnosis('current', 'healthy', 'Current demand is stable',
      `Median current changed ${pct(current.value)} in matched power bands.`,
      'No large increase in electrical effort is visible.',
      'Continue periodic comparison using the same battery and mechanical load.'));
  }

  if (response.value === null) {
    diagnoses.push(diagnosis('response', 'insufficient', 'Power-to-velocity response unavailable',
      'Matched power bins are insufficient.', 'The dashboard will not guess at degradation without comparable commands.',
      'Run a repeatable test that includes steady low, medium, and high power segments.'));
  } else {
    const responseSeverity: WearSeverity = response.value <= -15 ? 'warning' : response.value <= -8 ? 'watch' : 'healthy';
    ...
    diagnoses.push(diagnosis('response', responseSeverity, 'Command-to-motion response',
      `Battery-adjusted response changed ${pct(response.value)}.`, meaning, inspection));
  }

  if (stallChangePoints !== null && stallChangePoints >= 5) {
    diagnoses.push(diagnosis('overall', 'warning', 'Possible-stall exposure increased',
      `High-command, low-velocity samples increased by ${stallChangePoints.toFixed(1)} percentage points.`,
      'This pattern can occur with obstruction, binding, disconnected encoders, or a mechanism intentionally holding position.',
      'Review the matching time interval and inspect the mechanism before running it under prolonged load.'));
  }
...
export function buildTimeOverlay(
  run: Run,
  motor: string,
  metric: 'power' | 'velocity' | 'current',
): OverlayPoint[] {
