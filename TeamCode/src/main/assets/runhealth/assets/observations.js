 ...
 */
function countStallOccurrences(powerHistory, velocityHistory, thresholds) {
    let count = 0;
    let latest = -1;
    let vIdx = 0;
    for (let i = 0; i < powerHistory.length; i++) {
        const p = powerHistory[i];
        if (p.v == null || Math.abs(p.v) <= thresholds.stallPowerThreshold)
            continue;
        const t0 = p.t;
        // advance vIdx past anything older than t0
        while (vIdx < velocityHistory.length && velocityHistory[vIdx].t < t0)
            vIdx += 1;
        let anyFast = false;
 ...
 */
function assessBatteryDrop(batteryHistory) {
    let first = null;
    let last = null;
    let latestT = -1;
 ...
 */
function countLoopTimeSpikes(channelHistory, warnMs = 35) {
    let count = 0;
    let latest = -1;
    let prev = null;
...
function groupByKey(conds) {
    ...
    return Array.from(byKey.values()).sort((a, b) => b.latestTimestampMs - a.latestTimestampMs);
}
/**
 ...
 */
export function analyzeLiveConditions(store, _snap = null, thresholds = DEFAULT_THRESHOLDS) {
    const out = [];
    ...
    for (const name of store.getMotorNames()) {
        ...
        if (!m)
            continue;
        const { count, latest } = countStallOccurrences(m.powerHistory, m.velocityHistory, thresholds);
        if (count > 0) {
            out.push({
                key: `stall:${name}`,
                severity: 'warning',
